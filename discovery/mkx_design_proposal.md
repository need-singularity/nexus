...
- **단일 파일 IO 임계**: 18.9 MB 단일 텍스트 → grep 풀스캔 ~250ms (이미 빠른
  ...
  여전히 풀스캔.
- **Cross-project 연결 시들음**: anima / n6-architecture 와의 `@E` cross-ref
...
4. Mk.IX 까지 모두 nexus 단독 — **anima / n6-architecture / void** 와의 cross-ref
   automation 부재.
...
### 3.5 축 5 — Cross-Engine Bus (anima ↔ n6-architecture ↔ atlas.n6)

#### 가설
직전 commit `d266319e` 가 보여주듯 `@E cross-ref 엣지` 는 매뉴얼 1건씩 추가 됨.
**anima / n6-architecture / void / hexa-lang / airgenome / papers** 6 형제
