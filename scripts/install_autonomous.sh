...
do_uninstall() {
    ...
    if crontab -l 2>/dev/null | grep -q "$CRON_MARKER"; then
        ...
        crontab -l 2>/dev/null | grep -v "NEXUS-6\|nexus6_growth_daemon\|growth_daily_report\|nexus6.*push" | crontab -
    ...
    if [[ -f "$HOOK_PATH" ]] && grep -q "NEXUS-6" "$HOOK_PATH" 2>/dev/null; then
        ...
        local nexus_lines
        nexus_lines=$(grep -c "NEXUS-6\|nexus6\|measure\.sh" "$HOOK_PATH" 2>/dev/null || echo "0")
        if [[ "$nexus_lines" -ge $((line_count - 2)) ]]; then
            rm -f "$HOOK_PATH"
            info "Removed post-commit hook"
        else
            grep -v "NEXUS-6\|nexus6\|measure\.sh" "$HOOK_PATH" > "${HOOK_PATH}.tmp"
    ...
    echo ""
    info "Uninstall complete. Logs remain in $NEXUS6_HOME/ (remove manually if desired)."
    exit 0
}
...
echo "  State dir:  $NEXUS6_HOME"
echo ""

# ── 0. Create state directory ─────────────────────────────────────────
mkdir -p "$NEXUS6_HOME/reports"
info "State directory: $NEXUS6_HOME"
...
<dict>
    <key>Label</key>
    <string>com.nexus6.growth</string>
    <key>ProgramArguments</key>
    <array>
        <string>/bin/bash</string>
        <string>SCRIPT_DIR_PLACEHOLDER/nexus6_growth_daemon.sh</string>
    ...
    <string>HOME_PLACEHOLDER/.nexus6/growth.log</string>
    <key>StandardErrorPath</key>
    <string>HOME_PLACEHOLDER/.nexus6/growth-error.log</string>
...
*/30 * * * * cd $SCRIPT_DIR && PATH="$HOME/.local/bin:$HOME/.cargo/bin:\$PATH" bash nexus6_growth_daemon.sh --max-cycles 1 >> $NEXUS6_HOME/cron-growth.log 2>&1
# Daily deep growth: 6 cycles at 3am
0 3 * * * cd $SCRIPT_DIR && PATH="$HOME/.local/bin:$HOME/.cargo/bin:\$PATH" bash nexus6_growth_daemon.sh --max-cycles 6 >> $NEXUS6_HOME/deep-growth.log 2>&1
# Daily report: 7am
0 7 * * * cd $SCRIPT_DIR && PATH="$HOME/.local/bin:$HOME/.cargo/bin:\$PATH" bash growth_daily_report.sh >> $NEXUS6_HOME/daily-report.log 2>&1
# Weekly push: Sunday 4am
0 4 * * 0 cd $REPO_ROOT && git push origin main >> $NEXUS6_HOME/push.log 2>&1
# END NEXUS-6
CRON_EOF
)

# Remove existing nexus6 entries, then append
...
cd "$SCRIPT_DIR"
bash measure.sh >> "$NEXUS6_HOME/metrics-history.jsonl" 2>/dev/null &
HOOK_EOF
)
...
chmod +x "$SCRIPT_DIR/nexus6_growth_daemon.sh" 2>/dev/null || true
...
echo "    [4] state dir      : $NEXUS6_HOME/"
echo ""
echo "  To activate launchd (start autonomous growth):"
echo "    launchctl load $PLIST_PATH"
echo ""
echo "  To verify cron:"
echo "    crontab -l | grep nexus6"
echo ""
echo "  To uninstall everything:"
echo "    $SCRIPT_DIR/install_autonomous.sh --uninstall"
echo ""
echo "  Logs:"
echo "    $NEXUS6_HOME/growth.log           (launchd stdout)"
echo "    $NEXUS6_HOME/growth-error.log     (launchd stderr)"
echo "    $NEXUS6_HOME/cron-growth.log      (cron 30min cycles)"
echo "    $NEXUS6_HOME/deep-growth.log      (cron 3am deep cycles)"
echo "    $NEXUS6_HOME/daily-report.log     (daily report output)"
echo "    $NEXUS6_HOME/metrics-history.jsonl (git hook measurements)"
echo "    $NEXUS6_HOME/notifications.log    (growth notifications)"
echo ""
