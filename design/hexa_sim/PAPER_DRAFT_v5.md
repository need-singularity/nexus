...
date: "2026-04-26"
version: "v5 draft (arXiv prep; license + ORCID + metadata; v4 body byte-identical, raw 77)"
...
license-file: "LICENSE-CC-BY-4.0.md"
msc-primary: "11A25"
msc-secondary: ["03B30", "68V20"]
arxiv-primary: "math.HO"
arxiv-cross-list: ["cs.MS", "cs.LO"]
---

# n=6 as foundational invariant: a multi-domain falsifier-grounded framework

**Authors / Affiliation.** dancinlife, Independent Researcher (mk225599@proton.me; ORCID iD pending — register at <https://orcid.org>; follow-up commit per raw 77).
**Date.** 2026-04-26 (v5, arXiv prep). **License.** CC-BY-4.0; see `LICENSE-CC-BY-4.0.md` co-located.

---

## arXiv submission metadata

For the arXiv web form. Body (Abstract through §13.16) is byte-identical to
`PAPER_DRAFT_v4.md`.

**arXiv abstract (1,887 chars; under the 1,920 cap; ASCII form of the
canonical in-body §"Abstract" which uses LaTeX, $\approx 2{,}472$ chars).**

> We present a falsifier-grounded framework that examines whether the integer
> n=6 functions as a multi-domain invariant rather than a numerological
> accident. The framework couples six foundation primitives derived from the
> perfect-number structure of n=6 (sigma, varphi, tau, sopfr, J_2, mu) with
> 115 executable falsifiers organised across 11 atlas shards spanning number
> theory, chemistry, biology, particle physics, cosmology, and group theory.
> Each falsifier is a sentinel-emitting shell template whose 16-hex
> cmd_sha256 is registered at write-time (R1) and whose rotation history is
> recorded in a hash-chained append-only ledger signed by an SSH key (R5),
> giving a five-layer defense chain whose nine operational cells were
> validated end-to-end at 7/7 PASS. Cross-shard aggregation across four
> independent repositories yields 9,165 unique tuples with zero collisions
> over 333+ commits, and a 3/4 score on a six-precondition Honesty triad.
> Three load-bearing findings anchor the central claim: the Diophantine
> biconditional sigma(n)*varphi(n) = n*tau(n) iff n=6 (F100); the
> group-theoretic singularity Out(S_6) iso Z/2 (F75); and the codon
> triple-decomposition 64 = 2^n = 4^(n/2) = tau^3 (F36). We give equal weight
> to two declined claims (F45 cross-bridge 3.5% triplet and F95 v2
> correlation hunt p=0.84 at 5e4 Monte Carlo trials), demonstrating that
> rigorously falsified candidates are first-class evidence. A meta-analysis
> of registry growth reports a 12x collapse in promote-rate (45% to 3.9%)
> followed by a pivot from horizontal anchor expansion to vertical
> defense-layer addition, establishing saturation as a positive health
> signal. Every claim is recoverable from git log plus a falsifier sentinel
> plus an R5 ledger walk. We do not claim metaphysical primacy for n=6; we
> claim the empirical residue, after honest declines, exceeds a chance
> baseline and merits broader auditing.

**MSC2020.** Primary `11A25` (multiplicative arithmetic functions); secondary
`03B30` (higher-order arithmetic) + `68V20` (reproducibility).

**arXiv categories.** Primary `math.HO`; cross-list `cs.MS` (for §3 + §13) and
`cs.LO` (for the §3.6 admissibility rule and §8.2 defense audit).

**Comments field.** "v5 draft; ~14,400 words; 7 ASCII figures + 14-stage repro
protocol; 88 refs; CC-BY-4.0; companion data per §11.2."

**Pandoc smoke-test (pre-submit).**
...
grep -iE 'error|warning' /tmp/pandoc_v5.log | wc -l   # expect 0 or soft-only
```

Cross-ref / figure / LaTeX errors are pre-submit blockers. Figures are inline
ASCII; no external images.

**Readiness.** Author, CC-BY-4.0, MSC, categories, 1,887-char abstract, 88
refs, and 14-stage protocol in place; body byte-identical to v4. Pending:
optional ORCID; operator pandoc run; possible math.HO endorsement
(<https://arxiv.org/help/endorsement>); operator upload.
