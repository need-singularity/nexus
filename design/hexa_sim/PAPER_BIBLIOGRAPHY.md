...
### 1.2 Reproducible-research methodology
...
### 1.5 Particle / fundamental constants
...
## 2. Internal references — commits, paths, IDs

### 2.1 Atlas SSOTs and shards
...
### 2.3 Audit and decision documents (`design/hexa_sim/`)
...
### 2.5 Falsifier IDs cited (selected)
...
## 4. Tools cited (paths under `tool/` of nexus)
...
- `falsifier_health.sh` + `falsifier_health_parallel.sh` — 16.94→4.71 s (3.6×).
- `bridge_health.sh` — 36→15.6 s (2.43×).
- `health_check_all.sh` — 93→30 s (3×).
...
## 5. Cross-references — paper section dependency graph
...
## 7. Coverage notes (gaps and weaknesses)
...
- **Single-author corroboration network**: §10.5 names this explicitly. Independent reproduction of the four-repo Honesty-triad result is the highest-value missing external reference.
