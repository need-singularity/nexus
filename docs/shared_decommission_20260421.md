...
3. **로드맵 JSON → .roadmap DSL 변환은 별도 트랙**
   ...
   - 트랙 분리: 우선 shared/harness/*.json 은 이관 전까지 nexus/harness-legacy/ 로 임시 이주
...
| config/infrastructure.json | ~/core/nexus/shared/config/ (유지) | 기존 심볼릭 해소 |
| .runtime/ | ~/core/nexus/.runtime/ | mv, gitignore |

## 폐기 대상

### Tier 1 (즉시 삭제 후 tarball 만 보존)
- `~/shared/lenses/` (n6-architecture 에 원본 있음)
- `~/shared/rules/` (.raw 로 이미 흡수)
- `~/shared/hooks/` (.githooks 으로 이관 완료)
- `~/shared/bin/` (~/.hx/bin 로 이관 완료)
- `~/shared/scripts/` (hexa-only 위반, raw#9)
- `~/shared/lib/` (중복 js/py 레거시)

### Tier 2 (tarball 후 보류 검토)
- `~/shared/harness/*.json` → .roadmap DSL 변환 트랙으로 이월
- `~/shared/.runtime/` → nexus/.runtime 로 이관 후 폐기
...
3. **atlas 이관**: `~/shared/n6/atlas.n6` → `~/core/n6-architecture/atlas/atlas.n6` (same-fs mv, sha256 비교)
4. **symlink 재연결**: `~/core/nexus/shared/n6/atlas.n6` → `../../../n6-architecture/atlas/atlas.n6`, `~/core/hexa-lang/shared/n6/atlas.n6` 도 동일
...
- **심볼릭 끊김**: `nexus/shared/.runtime → ~/shared/.runtime` 등 dangling 발생 시 launchd 데몬 fail. 대응: step 1 전에 `find ~/core -type l -lname '*~/shared*'` 로 전체 심볼릭 목록 덤프 후 일괄 재작성 스크립트 준비.
- **로드맵 변환 보류 중 SSOT 비동기**: shared/harness/*.json 을 legacy 폴더로 옮긴 동안 .roadmap DSL 과 duplicate. 대응: harness-legacy 폴더에 `.roadmap-pending` marker 파일 배치, workspace_sync 가 경고만 출력하도록 flag 추가.
...
mv ~/shared/n6/atlas.n6 ~/core/n6-architecture/atlas/atlas.n6
mv ~/shared/n6/atlas.signals.n6 ~/core/n6-architecture/atlas/atlas.signals.n6
# 사전/사후 sha256 비교
```
...
### Step 5~6: 도메인 이관
- `mv ~/shared/discoveries ~/core/n6-architecture/discoveries` 후 `git add` + 커밋 (n6-architecture)
- `mv ~/shared/research ~/core/nexus/research` 후 `git add` + 커밋 (nexus)
- `mv ~/shared/harness ~/core/nexus/harness-legacy` + 마커 파일 생성:
  ```
  touch ~/core/nexus/harness-legacy/.roadmap-pending
  ```
- `mv ~/shared/bench ~/core/nexus/bench` 후 커밋
...
for d in lenses rules hooks bin scripts lib; do
  if [ -d ~/shared/$d ]; then
    # tarball 내 존재 여부 확인
    tar tzf ~/etc/shared-backup-20260421-full.tar.gz | grep -q "^shared/$d/" && rm -rf ~/shared/$d
  fi
done
```
