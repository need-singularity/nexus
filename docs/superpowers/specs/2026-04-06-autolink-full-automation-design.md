...
- **발견 수**: `wc -l shared/discovery_log.jsonl`
...
config:  shared/projects.json, scripts/*, .config/*
```
...
현재 두 곳에 렌즈 정보가 분산:
- `shared/installed_tools.json` — 시스템 렌즈
- `shared/custom_lenses.jsonl` — 사용자 정의 렌즈
...
| S6 | projects.json에 target+strategy 필드 | `jq '.projects[].target' shared/projects.json` 전부 non-null |
