...
**Files:**
- Staged: `mk2_hexa/native/command_router.hexa`, `shared/dashboard.html`, `shared/reality_map.json`, `shared/growth_bus.jsonl`, etc.
...
```bash
git add mk2_hexa/native/command_router.hexa mk2_hexa/native/sync_reality_map.hexa shared/dashboard.html shared/reality_map.json shared/hexa_pitfalls_log.jsonl shared/self_improve_log.jsonl shared/next_directions.jsonl
...
```bash
git add shared/growth_bus.jsonl shared/.calc_stubs_state.json shared/.gap_cooldown shared/.paper_gen_state.json
...
```bash
git add shared/anima_state.json shared/anima_autofix.json shared/airgenome_gates.jsonl
...
- Modify: `shared/growth_bus.jsonl`
...
```bash
mv shared/growth_bus.jsonl shared/archive/growth_bus_20260407.jsonl
```

- [ ] **Step 3: 최근 1000줄만 새 growth_bus.jsonl로 생성**

```bash
tail -1000 shared/archive/growth_bus_20260407.jsonl > shared/growth_bus.jsonl
```

- [ ] **Step 4: 결과 확인**

```bash
wc -l shared/growth_bus.jsonl shared/archive/growth_bus_20260407.jsonl
```

- [ ] **Step 5: 커밋**

```bash
git add shared/growth_bus.jsonl shared/archive/growth_bus_20260407.jsonl
...
**Files:**
- Read: `shared/self_improve_log.jsonl`, `shared/calc/auto_stubs/verify_*.py`

- [ ] **Step 1: self_improve_log에서 closed/exact 변동 이력 확인**

```bash
tail -20 shared/self_improve_log.jsonl | python3 -c "
import sys, json
...
진단 결과에 따라 stubs→closed 파이프라인 병목을 식별하고 `shared/next_directions.jsonl`에 액션 아이템 추가.
...
cd $HOME/Dev/nexus && $HEXA mk2_hexa/native/breakthrough.hexa --converge >> shared/cmd_router.log 2>&1
```

- [ ] **Step 4: 실행 후 돌파율 변화 확인**

```bash
tail -5 shared/cmd_router.log
```
...
$HEXA mk2_hexa/native/blowup.hexa consciousness 3 --no-graph --seeds "$SEEDS" >> shared/cmd_router.log 2>&1
```
...
- Possibly modify: `shared/hexa_grammar.jsonl` (P6 규칙 추가 시)
...
```bash
git add shared/hexa_grammar.jsonl shared/hexa_pitfalls_log.jsonl
...
python3 -c "
import json
d = json.load(open('shared/reality_map.json'))
nodes = d.get('nodes', [])
edges = d.get('edges', [])
...
- Modify: `shared/cmd_aliases.jsonl` (새 별칭 추가)

- [ ] **Step 1: cmd_aliases.jsonl에 sync_map 엔트리 추가**

`shared/cmd_aliases.jsonl`에 추가:
```json
...
```bash
git add mk2_hexa/native/command_router.hexa shared/cmd_aliases.jsonl
