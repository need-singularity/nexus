...
**Goal:** nexus가 동기화, 성장, 미러볼 스캔, 진화를 무인으로 반복 실행하는 완전 자율 데몬 모드 구현

**Architecture:** `nexus daemon` CLI 커맨드를 추가하여 기존 `loop` 명령을 무한 반복 + 적응 간격 + 시그널 핸들링으로 감싼다. Loop 자체는 기존 6단계에서 8단계로 확장 (mirror_scan + growth_bridge 추가). LaunchAgent plist를 daemon 모드로 전환하여 macOS 부팅 시 자동 시작.
...
| `~/Library/LaunchAgents/com.n6.sync.nexus.plist` | macOS 데몬 | daemon 모드로 변경 |
| `scripts/nexus-daemon.sh` | 데몬 래퍼 스크립트 | 신규 생성 |
...
Run: `cargo check --bin nexus 2>&1 | grep "^error"`
Expected: 에러 없음
...
fn run_daemon(domain: Option<String>, interval_min: u64, max_loops: Option<usize>) -> Result<(), String> {
    ...
    while running.load(Ordering::Relaxed) {
        ...
        // 리포트를 ~/.nexus/last_daemon_status.txt에도 저장
        let status_line = format!(
            "loop={} time={} next={}min domain={}",
            loop_count, now, interval_min, domain_str
        );
        let status_path = std::env::var("HOME")
            .map(|h| format!("{}/.nexus/last_daemon_status.txt", h))
            .unwrap_or_else(|_| "/tmp/nexus_daemon_status.txt".to_string());
...
fn run_daemon(domain: Option<String>, interval_min: u64, max_loops: Option<usize>) -> Result<(), String> {
    ...
    loop {
        ...
        let path = std::env::var("HOME")
            .map(|h| format!("{}/.nexus/daemon_status.txt", h))
            .unwrap_or_else(|_| "/tmp/nexus_daemon_status.txt".to_string());
...
**Files:**
- Create: `scripts/nexus-daemon.sh`
...
```bash
chmod +x scripts/nexus-daemon.sh
```

- [ ] **Step 3: Commit**

```bash
git add scripts/nexus-daemon.sh
...
**Files:**
- Modify: `~/Library/LaunchAgents/com.n6.sync.nexus.plist`

- [ ] **Step 1: 기존 plist 백업**

```bash
cp ~/Library/LaunchAgents/com.n6.sync.nexus.plist \
   ~/Library/LaunchAgents/com.n6.sync.nexus.plist.bak
```
...
<dict>
    <key>Label</key>
    <string>com.n6.sync.nexus</string>
    ...
    <string>/Users/ghost/Library/Logs/nexus/daemon.log</string>

    <key>StandardErrorPath</key>
    <string>/Users/ghost/Library/Logs/nexus/daemon.err</string>
...
- ProgramArguments → nexus-daemon.sh

- [ ] **Step 3: LaunchAgent 재로드**

```bash
launchctl unload ~/Library/LaunchAgents/com.n6.sync.nexus.plist 2>/dev/null
launchctl load ~/Library/LaunchAgents/com.n6.sync.nexus.plist
```
...
cat ~/Library/Logs/nexus/daemon.log | tail -5
cat ~/.nexus/daemon_status.txt
```
...
- [ ] `cargo check --bin nexus` — 에러 없음
- [ ] `nexus loop --cycles 1` — 8단계 모두 실행, mirror_scan 포함
- [ ] `nexus daemon --max-loops 1 --interval 0` — 1회 실행 후 종료
- [ ] `nexus daemon --max-loops 2 --interval 1` — 2회 실행, 1분 대기, 종료
- [ ] `launchctl list | grep n6` — daemon 등록 확인
- [ ] `cat ~/.nexus/daemon_status.txt` — 상태 파일 확인
- [ ] `cat ~/.nexus/last_loop_report.txt` — 리포트 파일 확인
